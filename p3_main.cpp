...
void catalog_clear_helper(){
  for_each(catalog.begin(), catalog.end(), [](Collection* collection){delete collection;});
  catalog.clear();
  collection_count = 0;
  return;
}
...
void input_save_All(){
  ...
  for_each(catalog.begin(), catalog.end(), [&output](Collection* collection){collection->save(output);});
  output.close();
...
void input_find_record_by_string(){
  ...
  for_each(library_title.begin(), library_title.end(), [&key, &found](Record* record){
