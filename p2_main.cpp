...
Record* find_record_by_title(const string& title){
  ...
  auto it = lower_bound(library_title.begin(),library_title.end(),&probe_record, Record_compare_title());
...
Record* find_record_by_id(const int id){
  ...
  auto it = lower_bound(library_id.begin(),library_id.end(),&probe_record, Record_compare_id());
...
Collection* find_collection_by_name(string& name){
  ...
  auto it = lower_bound(catalog.begin(), catalog.end(), &probe_collection, Collection_compare_name());
  if (it == catalog.end())
...
void input_restore_All(){
  ...
  try {
    ...
    for (int i = 0; i < num_collections; i++) {
      Collection collection_tmp(input, library_title);
      Collection* collection = new Collection(collection_tmp);
      add_collection(collection);
    }
    input.close();
